...
| **Perforce** | Connects to a Helix Core server, logs in (password piped to `p4 login` stdin — never stored), lists workspaces, and syncs with optional force/parallel. Launches P4V. |
| **Sync & Launch** | The UGS‑style workflow: sync latest from Perforce → rebuild changed engine code → launch the editor (optionally with a `.uproject`), each step toggleable. |
